...
function $Y717$export$default() {
  return !/^((?!chrome|android).)*safari/i.test($iVQY$export$default());
}
function $W8bA$export$default(element, includeScale, isFixedStrategy) {
  ...
  var scaleX = 1;
  var scaleY = 1;
  if (includeScale && $wsKO$export$isHTMLElement(element)) {
    scaleX = element.offsetWidth > 0 ? $FuL6$export$round(clientRect.width) / element.offsetWidth || 1 : 1;
    scaleY = element.offsetHeight > 0 ? $FuL6$export$round(clientRect.height) / element.offsetHeight || 1 : 1;
  }
  var _ref = $wsKO$export$isElement(element) ? $QiNa$export$default(element) : window,
    visualViewport = _ref.visualViewport;
  var addVisualOffsets = !$Y717$export$default() && isFixedStrategy;
...
function $sJcE$export$default(element) {
  ...
  return (($wsKO$export$isElement(element) ? element.ownerDocument :
...
function $IVKl$export$default(element) {
  ...
  return (
    ...
    $wsKO$export$isShadowRoot(element) ? element.host : null) ||
    // ShadowRoot detected
...
function $CUhI$var$getTrueOffsetParent(element) {
  if (!$wsKO$export$isHTMLElement(element) ||
...
function $CUhI$var$getContainingBlock(element) {
  var isFirefox = /firefox/i.test($iVQY$export$default());
  var isIE = /Trident/i.test($iVQY$export$default());
  ...
  if ($wsKO$export$isShadowRoot(currentNode)) {
...
function $FbIu$export$within(min, value, max) {
  return $FuL6$export$max(min, $FuL6$export$min(value, max));
}
function $FbIu$export$withinMaxClamp(min, value, max) {
  var v = $FbIu$export$within(min, value, max);
  return v > max ? max : v;
}
...
function $zJlD$var$arrow(_ref) {
  ...
  var offset = $FbIu$export$within(min, center, max); // Prevents breaking syntax highlighting...
...
function $ebtt$var$roundOffsetsByDPR(_ref) {
  ...
  return {
    x: $FuL6$export$round(x * dpr) / dpr || 0,
    y: $FuL6$export$round(y * dpr) / dpr || 0
  };
}
function $ebtt$export$mapToStyles(_ref2) {
  ...
  var _ref4 = roundOffsets === true ? $ebtt$var$roundOffsetsByDPR({
    x: x,
    y: y
  }) : {
    x: x,
...
function $ebtt$var$computeStyles(_ref5) {
...
function $E2lw$export$default(element, strategy) {
  ...
  if (visualViewport) {
    ...
    var layoutViewport = $Y717$export$default();
...
function $M7BJ$var$getInnerBoundingClientRect(element, strategy) {
  var rect = $W8bA$export$default(element, false, strategy === 'fixed');
...
function $M7BJ$var$getClientRectFromMixedType(element, clippingParent, strategy) {
  return clippingParent === $XHUo$export$viewport ? $rB0G$export$default($E2lw$export$default(element, strategy)) : $wsKO$export$isElement(clippingParent) ? $M7BJ$var$getInnerBoundingClientRect(clippingParent, strategy) : $rB0G$export$default($yEje$export$default($sJcE$export$default(element)));
...
function $M7BJ$export$default(element, boundary, rootBoundary, strategy) {
  ...
  var clippingRect = clippingParents.reduce(function (accRect, clippingParent) {
    var rect = $M7BJ$var$getClientRectFromMixedType(element, clippingParent, strategy);
    ...
    return accRect;
  }, $M7BJ$var$getClientRectFromMixedType(element, firstClippingParent, strategy));
...
function $Moke$export$default(state, options) {
  ...
  var clippingClientRect = $M7BJ$export$default($wsKO$export$isElement(element) ? element : element.contextElement || $sJcE$export$default(state.elements.popper), boundary, rootBoundary, strategy);
...
function $RwXk$var$preventOverflow(_ref) {
  ...
  if (checkMainAxis) {
    ...
    var arrowLen = $FbIu$export$within(0, referenceRect[len], arrowRect[len]);
    ...
    var preventedOffset = $FbIu$export$within(tether ? $FuL6$export$min(min, tetherMin) : min, offset, tether ? $FuL6$export$max(max, tetherMax) : max);
  ...
  if (checkAltAxis) {
    ...
    var isOriginSide = [$XHUo$export$top, $XHUo$export$left].indexOf(basePlacement) !== -1;
    var _offsetModifierValue = (_offsetModifierState$2 = offsetModifierState == null ? void 0 : offsetModifierState[altAxis]) != null ? _offsetModifierState$2 : 0;
    var _tetherMin = isOriginSide ? _min : _offset - referenceRect[_len] - popperRect[_len] - _offsetModifierValue + normalizedTetherOffsetValue.altAxis;
    var _tetherMax = isOriginSide ? _offset + referenceRect[_len] + popperRect[_len] - _offsetModifierValue - normalizedTetherOffsetValue.altAxis : _max;
    var _preventedOffset = tether && isOriginSide ? $FbIu$export$withinMaxClamp(_tetherMin, _offset, _tetherMax) : $FbIu$export$within(tether ? _tetherMin : _min, _offset, tether ? _tetherMax : _max);
...
function $FCaM$var$isElementScaled(element) {
  ...
  var scaleX = $FuL6$export$round(rect.width) / element.offsetWidth || 1;
  var scaleY = $FuL6$export$round(rect.height) / element.offsetHeight || 1;
...
function $FCaM$export$default(elementOrVirtualElement, offsetParent, isFixed) {
  ...
  var rect = $W8bA$export$default(elementOrVirtualElement, offsetParentIsScaled, isFixed);
  var scroll = {
    scrollLeft: 0,
    scrollTop: 0
  };
  ...
  if (isOffsetParentAnElement || !isOffsetParentAnElement && !isFixed) {
    if ($B1zX$export$default(offsetParent) !== 'body' ||
...
const $xqrD$var$TRANSITION_END = 'transitionend'; // Shout-out Angus Croll (https://goo.gl/pxwQGp)

const $xqrD$var$toType = object => {
...
const $xqrD$var$isElement = object => {
...
const $xqrD$var$getElement = object => {
  // it's a jQuery object or a node element
  if ($xqrD$var$isElement(object)) {
  ...
  if (document.readyState === 'loading') {
    ...
    if (!$xqrD$var$DOMContentLoadedCallbacks.length) {
      document.addEventListener('DOMContentLoaded', () => {
        for (const callback of $xqrD$var$DOMContentLoadedCallbacks) {
          callback();
        }
      });
    }
...
function $xqrD$var$makeEventUid(element, uid) {
  return uid && `${uid}::${$xqrD$var$uidEvent++}` || element.uidEvent || $xqrD$var$uidEvent++;
}
function $xqrD$var$getElementEvents(element) {
  const uid = $xqrD$var$makeEventUid(element);
...
function $xqrD$var$findHandler(events, callable, delegationSelector = null) {
  return Object.values(events).find(event => event.callable === callable && event.delegationSelector === delegationSelector);
}
function $xqrD$var$normalizeParameters(originalTypeEvent, handler, delegationFunction) {
  const isDelegated = typeof handler === 'string'; // todo: tooltip passes `false` instead of selector, so we need to check

  const callable = isDelegated ? delegationFunction : handler || delegationFunction;
  let typeEvent = $xqrD$var$getTypeEvent(originalTypeEvent);
  if (!$xqrD$var$nativeEvents.has(typeEvent)) {
    typeEvent = originalTypeEvent;
  }
  return [isDelegated, callable, typeEvent];
}
function $xqrD$var$addHandler(element, originalTypeEvent, handler, delegationFunction, oneOff) {
  if (typeof originalTypeEvent !== 'string' || !element) {
    return;
  }
  let [isDelegated, callable, typeEvent] = $xqrD$var$normalizeParameters(originalTypeEvent, handler, delegationFunction); // in case of mouseenter or mouseleave wrap the handler within a function that checks for its DOM position
  // this prevents the handler from being dispatched the same way as mouseover or mouseout does

  if (originalTypeEvent in $xqrD$var$customEvents) {
  ...
  const events = $xqrD$var$getElementEvents(element);
  const handlers = events[typeEvent] || (events[typeEvent] = {});
  const previousFunction = $xqrD$var$findHandler(handlers, callable, isDelegated ? handler : null);
  if (previousFunction) {
    previousFunction.oneOff = previousFunction.oneOff && oneOff;
    return;
  }
  const uid = $xqrD$var$makeEventUid(callable, originalTypeEvent.replace($xqrD$var$namespaceRegex, ''));
  const fn = isDelegated ? $xqrD$var$bootstrapDelegationHandler(element, handler, callable) : $xqrD$var$bootstrapHandler(element, callable);
...
function $xqrD$var$removeNamespacedHandlers(element, events, typeEvent, namespace) {
  ...
  for (const handlerKey of Object.keys(storeElementEvent)) {
    if (handlerKey.includes(namespace)) {
      ...
      $xqrD$var$removeHandler(element, events, typeEvent, event.callable, event.delegationSelector);
    }
  }
}
...
const $xqrD$var$EventHandler = {
  on(element, event, handler, delegationFunction) {
    $xqrD$var$addHandler(element, event, handler, delegationFunction, false);
  },
  one(element, event, handler, delegationFunction) {
    $xqrD$var$addHandler(element, event, handler, delegationFunction, true);
  },
  off(element, originalTypeEvent, handler, delegationFunction) {
    if (typeof originalTypeEvent !== 'string' || !element) {
      return;
    }
    const [isDelegated, callable, typeEvent] = $xqrD$var$normalizeParameters(originalTypeEvent, handler, delegationFunction);
    ...
    if (typeof callable !== 'undefined') {
      ...
      $xqrD$var$removeHandler(element, events, typeEvent, callable, isDelegated ? handler : null);
      return;
    }
    if (isNamespace) {
  ...
  trigger(element, event, args) {
    ...
    evt = $xqrD$var$hydrateObj(evt, args);
    if (defaultPrevented) {
      evt.preventDefault();
    }
    if (nativeDispatch) {
...
function $xqrD$var$hydrateObj(obj, meta) {
 ...
 * --------------------------------------------------------------------------
 */
function $xqrD$var$normalizeData(value) {
  if (value === 'true') {
    return true;
  }
  if (value === 'false') {
...
class $xqrD$var$Config {
  ...
  _mergeConfigObj(config, element) {
    const jsonConfig = $xqrD$var$isElement(element) ? $xqrD$var$Manipulator.getDataAttribute(element, 'config') : {}; // try to parse

    return {
      ...this.constructor.Default,
      ...(typeof jsonConfig === 'object' ? jsonConfig : {}),
      ...($xqrD$var$isElement(element) ? $xqrD$var$Manipulator.getDataAttributes(element) : {}),
  ...
  _typeCheckConfig(config, configTypes = this.constructor.DefaultType) {
    for (const property of Object.keys(configTypes)) {
      ...
      const valueType = $xqrD$var$isElement(value) ? 'element' : $xqrD$var$toType(value);
...
const $xqrD$var$VERSION = '5.2.3';
/**
 * Class definition
 */

class $xqrD$var$BaseComponent extends $xqrD$var$Config {
...
const $xqrD$var$NAME$f = 'alert';
const $xqrD$var$DATA_KEY$a = 'bs.alert';
const $xqrD$var$EVENT_KEY$b = `.${$xqrD$var$DATA_KEY$a}`;
...
const $xqrD$var$NAME$e = 'button';
const $xqrD$var$DATA_KEY$9 = 'bs.button';
const $xqrD$var$EVENT_KEY$a = `.${$xqrD$var$DATA_KEY$9}`;
...
const $xqrD$var$EVENT_CLICK_DATA_API$6 = `click${$xqrD$var$EVENT_KEY$a}${$xqrD$var$DATA_API_KEY$6}`;
/**
 * Class definition
 */
...
const $xqrD$var$NAME$d = 'swipe';
const $xqrD$var$EVENT_KEY$9 = '.bs.swipe';
...
const $xqrD$var$Default$c = {
  endCallback: null,
  leftCallback: null,
  rightCallback: null
};
const $xqrD$var$DefaultType$c = {
...
class $xqrD$var$Swipe extends $xqrD$var$Config {
  constructor(element, config) {
    super();
    this._element = element;
    if (!element || !$xqrD$var$Swipe.isSupported()) {
      return;
    }
    this._config = this._getConfig(config);
    this._deltaX = 0;
  ...
  static get Default() {
    return $xqrD$var$Default$c;
  }
  static get DefaultType() {
    return $xqrD$var$DefaultType$c;
  }
  static get NAME() {
  ...
  _end(event) {
    ...
    this._handleSwipe();
    $xqrD$var$execute(this._config.endCallback);
  }
  _move(event) {
  ...
  _handleSwipe() {
    ...
    $xqrD$var$execute(direction > 0 ? this._config.rightCallback : this._config.leftCallback);
  }
  _initEvents() {
    if (this._supportPointerEvents) {
      $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_POINTERDOWN, event => this._start(event));
      $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_POINTERUP, event => this._end(event));
      this._element.classList.add($xqrD$var$CLASS_NAME_POINTER_EVENT);
    } else {
      $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_TOUCHSTART, event => this._start(event));
      $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_TOUCHMOVE, event => this._move(event));
      $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_TOUCHEND, event => this._end(event));
    }
  }
  _eventIsPointerPenTouch(event) {
    return this._supportPointerEvents && (event.pointerType === $xqrD$var$POINTER_TYPE_PEN || event.pointerType === $xqrD$var$POINTER_TYPE_TOUCH);
  } // Static
...
const $xqrD$var$NAME$c = 'carousel';
const $xqrD$var$DATA_KEY$8 = 'bs.carousel';
const $xqrD$var$EVENT_KEY$8 = `.${$xqrD$var$DATA_KEY$8}`;
const $xqrD$var$DATA_API_KEY$5 = '.data-api';
const $xqrD$var$ARROW_LEFT_KEY$1 = 'ArrowLeft';
const $xqrD$var$ARROW_RIGHT_KEY$1 = 'ArrowRight';
...
const $xqrD$var$EVENT_SLIDE = `slide${$xqrD$var$EVENT_KEY$8}`;
const $xqrD$var$EVENT_SLID = `slid${$xqrD$var$EVENT_KEY$8}`;
const $xqrD$var$EVENT_KEYDOWN$1 = `keydown${$xqrD$var$EVENT_KEY$8}`;
const $xqrD$var$EVENT_MOUSEENTER$1 = `mouseenter${$xqrD$var$EVENT_KEY$8}`;
const $xqrD$var$EVENT_MOUSELEAVE$1 = `mouseleave${$xqrD$var$EVENT_KEY$8}`;
const $xqrD$var$EVENT_DRAG_START = `dragstart${$xqrD$var$EVENT_KEY$8}`;
const $xqrD$var$EVENT_LOAD_DATA_API$3 = `load${$xqrD$var$EVENT_KEY$8}${$xqrD$var$DATA_API_KEY$5}`;
const $xqrD$var$EVENT_CLICK_DATA_API$5 = `click${$xqrD$var$EVENT_KEY$8}${$xqrD$var$DATA_API_KEY$5}`;
...
const $xqrD$var$SELECTOR_ACTIVE_ITEM = $xqrD$var$SELECTOR_ACTIVE + $xqrD$var$SELECTOR_ITEM;
...
const $xqrD$var$KEY_TO_DIRECTION = {
  [$xqrD$var$ARROW_LEFT_KEY$1]: $xqrD$var$DIRECTION_RIGHT,
  [$xqrD$var$ARROW_RIGHT_KEY$1]: $xqrD$var$DIRECTION_LEFT
};
const $xqrD$var$Default$b = {
  interval: 5000,
  keyboard: true,
  pause: 'hover',
  ride: false,
...
class $xqrD$export$Carousel extends $xqrD$var$BaseComponent {
  constructor(element, config) {
    ...
    if (this._config.ride === $xqrD$var$CLASS_NAME_CAROUSEL) {
      this.cycle();
    }
  } // Getters

  static get Default() {
    return $xqrD$var$Default$b;
  }
  static get DefaultType() {
    return $xqrD$var$DefaultType$b;
  }
  static get NAME() {
  ...
  _maybeEnableCycle() {
    ...
    if (this._isSliding) {
      $xqrD$var$EventHandler.one(this._element, $xqrD$var$EVENT_SLID, () => this.cycle());
      return;
    }
    this.cycle();
  }
  ...
  _addEventListeners() {
    if (this._config.keyboard) {
      $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_KEYDOWN$1, event => this._keydown(event));
    }
    if (this._config.pause === 'hover') {
      $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_MOUSEENTER$1, () => this.pause());
      $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_MOUSELEAVE$1, () => this._maybeEnableCycle());
    }
    if (this._config.touch && $xqrD$var$Swipe.isSupported()) {
      this._addTouchEventListeners();
    }
  }
  _addTouchEventListeners() {
    for (const img of $xqrD$var$SelectorEngine.find($xqrD$var$SELECTOR_ITEM_IMG, this._element)) {
      $xqrD$var$EventHandler.on(img, $xqrD$var$EVENT_DRAG_START, event => event.preventDefault());
    }
    const endCallBack = () => {
      if (this._config.pause !== 'hover') {
        return;
      ...
      this.touchTimeout = setTimeout(() => this._maybeEnableCycle(), $xqrD$var$TOUCHEVENT_COMPAT_WAIT + this._config.interval);
    };
    const swipeConfig = {
      leftCallback: () => this._slide(this._directionToOrder($xqrD$var$DIRECTION_LEFT)),
      rightCallback: () => this._slide(this._directionToOrder($xqrD$var$DIRECTION_RIGHT)),
      endCallback: endCallBack
    };
    this._swipeHelper = new $xqrD$var$Swipe(this._element, swipeConfig);
  }
  _keydown(event) {
  ...
  _setActiveIndicatorElement(index) {
    ...
    const activeIndicator = $xqrD$var$SelectorEngine.findOne($xqrD$var$SELECTOR_ACTIVE, this._indicatorsElement);
    activeIndicator.classList.remove($xqrD$var$CLASS_NAME_ACTIVE$2);
    activeIndicator.removeAttribute('aria-current');
    const newActiveIndicator = $xqrD$var$SelectorEngine.findOne(`[data-bs-slide-to="${index}"]`, this._indicatorsElement);
    if (newActiveIndicator) {
      newActiveIndicator.classList.add($xqrD$var$CLASS_NAME_ACTIVE$2);
  ...
  _slide(order, element = null) {
    ...
    const nextElement = element || $xqrD$var$getNextActiveElement(this._getItems(), activeElement, isNext, this._config.wrap);
    if (nextElement === activeElement) {
      return;
    }
    const nextElementIndex = this._getItemIndex(nextElement);
    const triggerEvent = eventName => {
      return $xqrD$var$EventHandler.trigger(this._element, eventName, {
    ...
    const slideEvent = triggerEvent($xqrD$var$EVENT_SLIDE);
    ...
    const completeCallBack = () => {
      ...
      this._isSliding = false;
      triggerEvent($xqrD$var$EVENT_SLID);
    };
  ...
  _isAnimated() {
    return this._element.classList.contains($xqrD$var$CLASS_NAME_SLIDE);
  }
  _getActive() {
    return $xqrD$var$SelectorEngine.findOne($xqrD$var$SELECTOR_ACTIVE_ITEM, this._element);
  }
  _getItems() {
    return $xqrD$var$SelectorEngine.find($xqrD$var$SELECTOR_ITEM, this._element);
  }
  _clearInterval() {
    if (this._interval) {
  ...
  static jQueryInterface(config) {
    return this.each(function () {
      const data = $xqrD$export$Carousel.getOrCreateInstance(this, config);
...
$xqrD$var$EventHandler.on(document, $xqrD$var$EVENT_CLICK_DATA_API$5, $xqrD$var$SELECTOR_DATA_SLIDE, function (event) {
  const target = $xqrD$var$getElementFromSelector(this);
  if (!target || !target.classList.contains($xqrD$var$CLASS_NAME_CAROUSEL)) {
    return;
  }
  event.preventDefault();
  const carousel = $xqrD$export$Carousel.getOrCreateInstance(target);
  ...
  if ($xqrD$var$Manipulator.getDataAttribute(this, 'slide') === 'next') {
    carousel.next();
    carousel._maybeEnableCycle();
    return;
  }
...
$xqrD$var$EventHandler.on(window, $xqrD$var$EVENT_LOAD_DATA_API$3, () => {
  const carousels = $xqrD$var$SelectorEngine.find($xqrD$var$SELECTOR_DATA_RIDE);
  for (const carousel of carousels) {
    $xqrD$export$Carousel.getOrCreateInstance(carousel);
  }
});
/**
 * jQuery
...
const $xqrD$var$NAME$b = 'collapse';
const $xqrD$var$DATA_KEY$7 = 'bs.collapse';
const $xqrD$var$EVENT_KEY$7 = `.${$xqrD$var$DATA_KEY$7}`;
const $xqrD$var$DATA_API_KEY$4 = '.data-api';
const $xqrD$var$EVENT_SHOW$6 = `show${$xqrD$var$EVENT_KEY$7}`;
const $xqrD$var$EVENT_SHOWN$6 = `shown${$xqrD$var$EVENT_KEY$7}`;
const $xqrD$var$EVENT_HIDE$6 = `hide${$xqrD$var$EVENT_KEY$7}`;
const $xqrD$var$EVENT_HIDDEN$6 = `hidden${$xqrD$var$EVENT_KEY$7}`;
const $xqrD$var$EVENT_CLICK_DATA_API$4 = `click${$xqrD$var$EVENT_KEY$7}${$xqrD$var$DATA_API_KEY$4}`;
...
class $xqrD$export$Collapse extends $xqrD$var$BaseComponent {
  constructor(element, config) {
    ...
    for (const elem of toggleList) {
      ...
      const filterElement = $xqrD$var$SelectorEngine.find(selector).filter(foundElement => foundElement === this._element);
  ...
  show() {
    ...
    if (this._config.parent) {
      activeChildren = this._getFirstLevelChildren($xqrD$var$SELECTOR_ACTIVES).filter(element => element !== this._element).map(element => $xqrD$export$Collapse.getOrCreateInstance(element, {
        toggle: false
      }));
    }
    ...
    const startEvent = $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_SHOW$6);
    ...
    const complete = () => {
      ...
      $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_SHOWN$6);
    };
  ...
  hide() {
    ...
    const startEvent = $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_HIDE$6);
    if (startEvent.defaultPrevented) {
      return;
    }
    const dimension = this._getDimension();
    ...
    for (const trigger of this._triggerArray) {
      const element = $xqrD$var$getElementFromSelector(trigger);
    ...
    const complete = () => {
      ...
      $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_HIDDEN$6);
    };
    this._element.style[dimension] = '';
    this._queueCallback(complete, this._element, true);
  }
  ...
  _initializeChildren() {
    ...
    const children = this._getFirstLevelChildren($xqrD$var$SELECTOR_DATA_TOGGLE$4);
  ...
  _getFirstLevelChildren(selector) {
    const children = $xqrD$var$SelectorEngine.find($xqrD$var$CLASS_NAME_DEEPER_CHILDREN, this._config.parent); // remove children if greater depth

    return $xqrD$var$SelectorEngine.find(selector, this._config.parent).filter(element => !children.includes(element));
  }
  _addAriaAndCollapsedClass(triggerArray, isOpen) {
    ...
    for (const element of triggerArray) {
      element.classList.toggle($xqrD$var$CLASS_NAME_COLLAPSED, !isOpen);
...
const $xqrD$var$NAME$a = 'dropdown';
const $xqrD$var$DATA_KEY$6 = 'bs.dropdown';
...
const $xqrD$var$ARROW_UP_KEY$1 = 'ArrowUp';
const $xqrD$var$ARROW_DOWN_KEY$1 = 'ArrowDown';
const $xqrD$var$RIGHT_MOUSE_BUTTON = 2; // MouseEvent.button value for the secondary button, usually the right button

const $xqrD$var$EVENT_HIDE$5 = `hide${$xqrD$var$EVENT_KEY$6}`;
const $xqrD$var$EVENT_HIDDEN$5 = `hidden${$xqrD$var$EVENT_KEY$6}`;
const $xqrD$var$EVENT_SHOW$5 = `show${$xqrD$var$EVENT_KEY$6}`;
const $xqrD$var$EVENT_SHOWN$5 = `shown${$xqrD$var$EVENT_KEY$6}`;
const $xqrD$var$EVENT_CLICK_DATA_API$3 = `click${$xqrD$var$EVENT_KEY$6}${$xqrD$var$DATA_API_KEY$3}`;
const $xqrD$var$EVENT_KEYDOWN_DATA_API = `keydown${$xqrD$var$EVENT_KEY$6}${$xqrD$var$DATA_API_KEY$3}`;
const $xqrD$var$EVENT_KEYUP_DATA_API = `keyup${$xqrD$var$EVENT_KEY$6}${$xqrD$var$DATA_API_KEY$3}`;
...
const $xqrD$var$CLASS_NAME_DROPUP_CENTER = 'dropup-center';
const $xqrD$var$CLASS_NAME_DROPDOWN_CENTER = 'dropdown-center';
const $xqrD$var$SELECTOR_DATA_TOGGLE$3 = '[data-bs-toggle="dropdown"]:not(.disabled):not(:disabled)';
const $xqrD$var$SELECTOR_DATA_TOGGLE_SHOWN = `${$xqrD$var$SELECTOR_DATA_TOGGLE$3}.${$xqrD$var$CLASS_NAME_SHOW$6}`;
const $xqrD$var$SELECTOR_MENU = '.dropdown-menu';
const $xqrD$var$SELECTOR_NAVBAR = '.navbar';
...
const $xqrD$var$PLACEMENT_TOPCENTER = 'top';
const $xqrD$var$PLACEMENT_BOTTOMCENTER = 'bottom';
...
class $xqrD$export$Dropdown extends $xqrD$var$BaseComponent {
  constructor(element, config) {
    ...
    this._menu = $xqrD$var$SelectorEngine.next(this._element, $xqrD$var$SELECTOR_MENU)[0] || $xqrD$var$SelectorEngine.prev(this._element, $xqrD$var$SELECTOR_MENU)[0] || $xqrD$var$SelectorEngine.findOne($xqrD$var$SELECTOR_MENU, this._parent);
  ...
  show() {
    if ($xqrD$var$isDisabled(this._element) || this._isShown()) {
      return;
    }
    const relatedTarget = {
      relatedTarget: this._element
    };
    const showEvent = $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_SHOW$5, relatedTarget);
    ...
    if ('ontouchstart' in document.documentElement && !this._parent.closest($xqrD$var$SELECTOR_NAVBAR_NAV)) {
    ...
    $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_SHOWN$5, relatedTarget);
  }
  hide() {
    if ($xqrD$var$isDisabled(this._element) || !this._isShown()) {
      return;
  ...
  _completeHide(relatedTarget) {
    const hideEvent = $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_HIDE$5, relatedTarget);
    ...
    if ('ontouchstart' in document.documentElement) {
      for (const element of [].concat(...document.body.children)) {
        $xqrD$var$EventHandler.off(element, 'mouseover', $xqrD$var$noop);
      }
    }
    if (this._popper) {
      this._popper.destroy();
    ...
    $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_HIDDEN$5, relatedTarget);
  }
  _getConfig(config) {
    ...
    if (typeof config.reference === 'object' && !$xqrD$var$isElement(config.reference) && typeof config.reference.getBoundingClientRect !== 'function') {
      ...
      throw new TypeError(`${$xqrD$var$NAME$a.toUpperCase()}: Option "reference" provided type "object" without a required "getBoundingClientRect" method.`);
    }
    return config;
  }
  _createPopper() {
  ...
  _isShown() {
    return this._menu.classList.contains($xqrD$var$CLASS_NAME_SHOW$6);
  }
  _getPlacement() {
    ...
    if (parentDropdown.classList.contains($xqrD$var$CLASS_NAME_DROPUP_CENTER)) {
      return $xqrD$var$PLACEMENT_TOPCENTER;
    }
    if (parentDropdown.classList.contains($xqrD$var$CLASS_NAME_DROPDOWN_CENTER)) {
      return $xqrD$var$PLACEMENT_BOTTOMCENTER;
  ...
  _detectNavbar() {
    return this._element.closest($xqrD$var$SELECTOR_NAVBAR) !== null;
  }
  _getOffset() {
    const {
      offset
  ...
  _getPopperConfig() {
    ...
    if (this._inNavbar || this._config.display === 'static') {
      $xqrD$var$Manipulator.setDataAttribute(this._menu, 'popper', 'static'); // todo:v6 remove
  ...
  _selectMenuItem({
    ...
    const items = $xqrD$var$SelectorEngine.find($xqrD$var$SELECTOR_VISIBLE_ITEMS, this._menu).filter(element => $xqrD$var$isVisible(element));
    if (!items.length) {
      return;
    } // if target isn't included in items (e.g. when expanding the dropdown)
    // allow cycling to get the last item in case key equals ARROW_UP_KEY

    $xqrD$var$getNextActiveElement(items, target, key === $xqrD$var$ARROW_DOWN_KEY$1, !items.includes(target)).focus();
  } // Static
  ...
  static clearMenus(event) {
    if (event.button === $xqrD$var$RIGHT_MOUSE_BUTTON || event.type === 'keyup' && event.key !== $xqrD$var$TAB_KEY$1) {
      return;
    }
    const openToggles = $xqrD$var$SelectorEngine.find($xqrD$var$SELECTOR_DATA_TOGGLE_SHOWN);
    for (const toggle of openToggles) {
      const context = $xqrD$export$Dropdown.getInstance(toggle);
  ...
  static dataApiKeydownHandler(event) {
    ...
    const isEscapeEvent = event.key === $xqrD$var$ESCAPE_KEY$2;
    const isUpOrDownEvent = [$xqrD$var$ARROW_UP_KEY$1, $xqrD$var$ARROW_DOWN_KEY$1].includes(event.key);
    ...
    const getToggleButton = this.matches($xqrD$var$SELECTOR_DATA_TOGGLE$3) ? this : $xqrD$var$SelectorEngine.prev(this, $xqrD$var$SELECTOR_DATA_TOGGLE$3)[0] || $xqrD$var$SelectorEngine.next(this, $xqrD$var$SELECTOR_DATA_TOGGLE$3)[0] || $xqrD$var$SelectorEngine.findOne($xqrD$var$SELECTOR_DATA_TOGGLE$3, event.delegateTarget.parentNode);
...
const $xqrD$var$PROPERTY_PADDING = 'padding-right';
const $xqrD$var$PROPERTY_MARGIN = 'margin-right';
/**
 * Class definition
 */
...
class $xqrD$var$ScrollBarHelper {
  ...
  hide() {
    ...
    this._setElementAttributes(this._element, $xqrD$var$PROPERTY_PADDING, calculatedValue => calculatedValue + width); // trick: We adjust positive paddingRight and negative marginRight to sticky-top elements to keep showing fullwidth

    this._setElementAttributes($xqrD$var$SELECTOR_FIXED_CONTENT, $xqrD$var$PROPERTY_PADDING, calculatedValue => calculatedValue + width);
    this._setElementAttributes($xqrD$var$SELECTOR_STICKY_CONTENT, $xqrD$var$PROPERTY_MARGIN, calculatedValue => calculatedValue - width);
  }
  reset() {
    this._resetElementAttributes(this._element, 'overflow');
    this._resetElementAttributes(this._element, $xqrD$var$PROPERTY_PADDING);
    this._resetElementAttributes($xqrD$var$SELECTOR_FIXED_CONTENT, $xqrD$var$PROPERTY_PADDING);
    this._resetElementAttributes($xqrD$var$SELECTOR_STICKY_CONTENT, $xqrD$var$PROPERTY_MARGIN);
  }
  isOverflowing() {
  ...
  _saveInitialAttribute(element, styleProperty) {
    ...
    if (actualValue) {
      $xqrD$var$Manipulator.setDataAttribute(element, styleProperty, actualValue);
    }
  }
  _resetElementAttributes(selector, styleProperty) {
    const manipulationCallBack = element => {
      const value = $xqrD$var$Manipulator.getDataAttribute(element, styleProperty); // We only want to remove the property if the value is `null`; the value can also be zero

      if (value === null) {
        element.style.removeProperty(styleProperty);
        return;
      }
      $xqrD$var$Manipulator.removeDataAttribute(element, styleProperty);
  ...
  _applyManipulationCallback(selector, callBack) {
    ...
    for (const sel of $xqrD$var$SelectorEngine.find(selector, this._element)) {
      callBack(sel);
    }
  }
}
...
const $xqrD$var$NAME$9 = 'backdrop';
const $xqrD$var$CLASS_NAME_FADE$4 = 'fade';
const $xqrD$var$CLASS_NAME_SHOW$5 = 'show';
const $xqrD$var$EVENT_MOUSEDOWN = `mousedown.bs.${$xqrD$var$NAME$9}`;
...
class $xqrD$var$Backdrop extends $xqrD$var$Config {
  constructor(config) {
    super();
    this._config = this._getConfig(config);
  ...
  show(callback) {
    ...
    element.classList.add($xqrD$var$CLASS_NAME_SHOW$5);
  ...
  _append() {
    ...
    $xqrD$var$EventHandler.on(element, $xqrD$var$EVENT_MOUSEDOWN, () => {
...
const $xqrD$var$NAME$8 = 'focustrap';
const $xqrD$var$DATA_KEY$5 = 'bs.focustrap';
const $xqrD$var$EVENT_KEY$5 = `.${$xqrD$var$DATA_KEY$5}`;
const $xqrD$var$EVENT_FOCUSIN$2 = `focusin${$xqrD$var$EVENT_KEY$5}`;
const $xqrD$var$EVENT_KEYDOWN_TAB = `keydown.tab${$xqrD$var$EVENT_KEY$5}`;
...
class $xqrD$var$FocusTrap extends $xqrD$var$Config {
  constructor(config) {
    super();
    this._config = this._getConfig(config);
  ...
  static get DefaultType() {
    return $xqrD$var$DefaultType$7;
  }
  static get NAME() {
    return $xqrD$var$NAME$8;
  } // Public
  ...
  activate() {
    ...
    $xqrD$var$EventHandler.off(document, $xqrD$var$EVENT_KEY$5); // guard against infinite focus loop

    $xqrD$var$EventHandler.on(document, $xqrD$var$EVENT_FOCUSIN$2, event => this._handleFocusin(event));
  ...
  deactivate() {
    ...
    this._isActive = false;
    $xqrD$var$EventHandler.off(document, $xqrD$var$EVENT_KEY$5);
  } // Private
...
const $xqrD$var$NAME$7 = 'modal';
const $xqrD$var$DATA_KEY$4 = 'bs.modal';
const $xqrD$var$EVENT_KEY$4 = `.${$xqrD$var$DATA_KEY$4}`;
const $xqrD$var$DATA_API_KEY$2 = '.data-api';
const $xqrD$var$ESCAPE_KEY$1 = 'Escape';
const $xqrD$var$EVENT_HIDE$4 = `hide${$xqrD$var$EVENT_KEY$4}`;
const $xqrD$var$EVENT_HIDE_PREVENTED$1 = `hidePrevented${$xqrD$var$EVENT_KEY$4}`;
const $xqrD$var$EVENT_HIDDEN$4 = `hidden${$xqrD$var$EVENT_KEY$4}`;
const $xqrD$var$EVENT_SHOW$4 = `show${$xqrD$var$EVENT_KEY$4}`;
const $xqrD$var$EVENT_SHOWN$4 = `shown${$xqrD$var$EVENT_KEY$4}`;
const $xqrD$var$EVENT_RESIZE$1 = `resize${$xqrD$var$EVENT_KEY$4}`;
const $xqrD$var$EVENT_CLICK_DISMISS = `click.dismiss${$xqrD$var$EVENT_KEY$4}`;
const $xqrD$var$EVENT_MOUSEDOWN_DISMISS = `mousedown.dismiss${$xqrD$var$EVENT_KEY$4}`;
const $xqrD$var$EVENT_KEYDOWN_DISMISS$1 = `keydown.dismiss${$xqrD$var$EVENT_KEY$4}`;
const $xqrD$var$EVENT_CLICK_DATA_API$2 = `click${$xqrD$var$EVENT_KEY$4}${$xqrD$var$DATA_API_KEY$2}`;
...
class $xqrD$export$Modal extends $xqrD$var$BaseComponent {
  ...
  show(relatedTarget) {
    ...
    const showEvent = $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_SHOW$4, {
      relatedTarget
    });
    if (showEvent.defaultPrevented) {
      return;
  ...
  hide() {
    ...
    const hideEvent = $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_HIDE$4);
    if (hideEvent.defaultPrevented) {
      return;
    }
    this._isShown = false;
  ...
  dispose() {
    for (const htmlElement of [window, this._dialog]) {
      $xqrD$var$EventHandler.off(htmlElement, $xqrD$var$EVENT_KEY$4);
    }
  ...
  _showElement(relatedTarget) {
    ...
    const transitionComplete = () => {
      ...
      this._isTransitioning = false;
      $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_SHOWN$4, {
        relatedTarget
      });
    };
  ...
  _addEventListeners() {
    $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_KEYDOWN_DISMISS$1, event => {
      if (event.key !== $xqrD$var$ESCAPE_KEY$1) {
        return;
      }
      if (this._config.keyboard) {
        event.preventDefault();
    ...
    $xqrD$var$EventHandler.on(window, $xqrD$var$EVENT_RESIZE$1, () => {
      if (this._isShown && !this._isTransitioning) {
        this._adjustDialog();
      }
    });
    $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_MOUSEDOWN_DISMISS, event => {
      // a bad trick to segregate clicks that may start inside dialog but end outside, and avoid listen to scrollbar clicks
      $xqrD$var$EventHandler.one(this._element, $xqrD$var$EVENT_CLICK_DISMISS, event2 => {
  ...
  _hideModal() {
    ...
    this._backdrop.hide(() => {
      ...
      $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_HIDDEN$4);
    });
  }
  _isAnimated() {
  ...
  _triggerBackdropTransition() {
    const hideEvent = $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_HIDE_PREVENTED$1);
    if (hideEvent.defaultPrevented) {
      return;
    }
    ...
    if (initialOverflowY === 'hidden' || this._element.classList.contains($xqrD$var$CLASS_NAME_STATIC)) {
      return;
    }
  ...
  _adjustDialog() {
    ...
    if (isBodyOverflowing && !isModalOverflowing) {
      const property = $xqrD$var$isRTL() ? 'paddingLeft' : 'paddingRight';
      this._element.style[property] = `${scrollbarWidth}px`;
    }
    if (!isBodyOverflowing && isModalOverflowing) {
      const property = $xqrD$var$isRTL() ? 'paddingRight' : 'paddingLeft';
      this._element.style[property] = `${scrollbarWidth}px`;
    }
  }
  _resetAdjustments() {
...
$xqrD$var$EventHandler.on(document, $xqrD$var$EVENT_CLICK_DATA_API$2, $xqrD$var$SELECTOR_DATA_TOGGLE$2, function (event) {
  ...
  $xqrD$var$EventHandler.one(target, $xqrD$var$EVENT_SHOW$4, showEvent => {
    if (showEvent.defaultPrevented) {
      // only register focus restorer if modal will actually get shown
      return;
    }
    $xqrD$var$EventHandler.one(target, $xqrD$var$EVENT_HIDDEN$4, () => {
  ...
  const alreadyOpen = $xqrD$var$SelectorEngine.findOne($xqrD$var$OPEN_SELECTOR$1);
  if (alreadyOpen) {
    $xqrD$export$Modal.getInstance(alreadyOpen).hide();
  }
...
const $xqrD$var$NAME$6 = 'offcanvas';
const $xqrD$var$DATA_KEY$3 = 'bs.offcanvas';
const $xqrD$var$EVENT_KEY$3 = `.${$xqrD$var$DATA_KEY$3}`;
const $xqrD$var$DATA_API_KEY$1 = '.data-api';
const $xqrD$var$EVENT_LOAD_DATA_API$2 = `load${$xqrD$var$EVENT_KEY$3}${$xqrD$var$DATA_API_KEY$1}`;
const $xqrD$var$ESCAPE_KEY = 'Escape';
const $xqrD$var$CLASS_NAME_SHOW$3 = 'show';
const $xqrD$var$CLASS_NAME_SHOWING$1 = 'showing';
const $xqrD$var$CLASS_NAME_HIDING = 'hiding';
const $xqrD$var$CLASS_NAME_BACKDROP = 'offcanvas-backdrop';
const $xqrD$var$OPEN_SELECTOR = '.offcanvas.show';
const $xqrD$var$EVENT_SHOW$3 = `show${$xqrD$var$EVENT_KEY$3}`;
const $xqrD$var$EVENT_SHOWN$3 = `shown${$xqrD$var$EVENT_KEY$3}`;
const $xqrD$var$EVENT_HIDE$3 = `hide${$xqrD$var$EVENT_KEY$3}`;
const $xqrD$var$EVENT_HIDE_PREVENTED = `hidePrevented${$xqrD$var$EVENT_KEY$3}`;
const $xqrD$var$EVENT_HIDDEN$3 = `hidden${$xqrD$var$EVENT_KEY$3}`;
const $xqrD$var$EVENT_RESIZE = `resize${$xqrD$var$EVENT_KEY$3}`;
const $xqrD$var$EVENT_CLICK_DATA_API$1 = `click${$xqrD$var$EVENT_KEY$3}${$xqrD$var$DATA_API_KEY$1}`;
const $xqrD$var$EVENT_KEYDOWN_DISMISS = `keydown.dismiss${$xqrD$var$EVENT_KEY$3}`;
...
class $xqrD$export$Offcanvas extends $xqrD$var$BaseComponent {
  ...
  show(relatedTarget) {
    ...
    this._element.setAttribute('aria-modal', true);
    this._element.setAttribute('role', 'dialog');
    this._element.classList.add($xqrD$var$CLASS_NAME_SHOWING$1);
    const completeCallBack = () => {
      if (!this._config.scroll || this._config.backdrop) {
        this._focustrap.activate();
      }
      this._element.classList.add($xqrD$var$CLASS_NAME_SHOW$3);
      this._element.classList.remove($xqrD$var$CLASS_NAME_SHOWING$1);
  ...
  hide() {
    ...
    this._element.blur();
    this._isShown = false;
    this._element.classList.add($xqrD$var$CLASS_NAME_HIDING);
    this._backdrop.hide();
    const completeCallback = () => {
      this._element.classList.remove($xqrD$var$CLASS_NAME_SHOW$3, $xqrD$var$CLASS_NAME_HIDING);
      this._element.removeAttribute('aria-modal');
      this._element.removeAttribute('role');
      if (!this._config.scroll) {
  ...
  _initializeBackDrop() {
    const clickCallback = () => {
      if (this._config.backdrop === 'static') {
        $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_HIDE_PREVENTED);
        return;
      }
      this.hide();
    }; // 'static' option will be translated to true, and booleans will keep their value
  ...
  _addEventListeners() {
    $xqrD$var$EventHandler.on(this._element, $xqrD$var$EVENT_KEYDOWN_DISMISS, event => {
      if (event.key !== $xqrD$var$ESCAPE_KEY) {
        return;
      }
      if (!this._config.keyboard) {
        $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_HIDE_PREVENTED);
...
$xqrD$var$EventHandler.on(document, $xqrD$var$EVENT_CLICK_DATA_API$1, $xqrD$var$SELECTOR_DATA_TOGGLE$1, function (event) {
  ...
  const alreadyOpen = $xqrD$var$SelectorEngine.findOne($xqrD$var$OPEN_SELECTOR);
  if (alreadyOpen && alreadyOpen !== target) {
    $xqrD$export$Offcanvas.getInstance(alreadyOpen).hide();
  }
...
$xqrD$var$EventHandler.on(window, $xqrD$var$EVENT_LOAD_DATA_API$2, () => {
  for (const selector of $xqrD$var$SelectorEngine.find($xqrD$var$OPEN_SELECTOR)) {
    $xqrD$export$Offcanvas.getOrCreateInstance(selector).show();
  }
});
$xqrD$var$EventHandler.on(window, $xqrD$var$EVENT_RESIZE, () => {
  for (const element of $xqrD$var$SelectorEngine.find('[aria-modal][class*=show][class*=offcanvas-]')) {
    if (getComputedStyle(element).position !== 'fixed') {
      $xqrD$export$Offcanvas.getOrCreateInstance(element).hide();
    }
  }
});
...
function $xqrD$var$sanitizeHtml(unsafeHtml, allowList, sanitizeFunction) {
...
const $xqrD$var$NAME$5 = 'TemplateFactory';
...
const $xqrD$var$DefaultContentType = {
...
class $xqrD$var$TemplateFactory extends $xqrD$var$Config {
  constructor(config) {
    super();
    this._config = this._getConfig(config);
  } // Getters
  ...
  _setContent(template, content, selector) {
    ...
    if ($xqrD$var$isElement(content)) {
      this._putElementInTemplate($xqrD$var$getElement(content), templateElement);
      return;
    }
  ...
  _maybeSanitize(arg) {
    return this._config.sanitize ? $xqrD$var$sanitizeHtml(arg, this._config.allowList, this._config.sanitizeFn) : arg;
  }
...
const $xqrD$var$EVENT_HIDE$2 = 'hide';
const $xqrD$var$EVENT_HIDDEN$2 = 'hidden';
const $xqrD$var$EVENT_SHOW$2 = 'show';
const $xqrD$var$EVENT_SHOWN$2 = 'shown';
const $xqrD$var$EVENT_INSERTED = 'inserted';
const $xqrD$var$EVENT_CLICK$1 = 'click';
const $xqrD$var$EVENT_FOCUSIN$1 = 'focusin';
const $xqrD$var$EVENT_FOCUSOUT$1 = 'focusout';
const $xqrD$var$EVENT_MOUSEENTER = 'mouseenter';
const $xqrD$var$EVENT_MOUSELEAVE = 'mouseleave';
...
class $xqrD$export$Tooltip extends $xqrD$var$BaseComponent {
  ...
  show() {
    ...
    const showEvent = $xqrD$var$EventHandler.trigger(this._element, this.constructor.eventName($xqrD$var$EVENT_SHOW$2));
    ...
    if (!this._element.ownerDocument.documentElement.contains(this.tip)) {
      ...
      $xqrD$var$EventHandler.trigger(this._element, this.constructor.eventName($xqrD$var$EVENT_INSERTED));
    }
    this._popper = this._createPopper(tip);
    tip.classList.add($xqrD$var$CLASS_NAME_SHOW$2); // If this is a touch-enabled device we add extra
    // empty mouseover listeners to the body's immediate children;
    // only needed because of broken event delegation on iOS
    // https://www.quirksmode.org/blog/archives/2014/02/mouse_event_bub.html
    ...
    const complete = () => {
      $xqrD$var$EventHandler.trigger(this._element, this.constructor.eventName($xqrD$var$EVENT_SHOWN$2));
  ...
  hide() {
    ...
    const hideEvent = $xqrD$var$EventHandler.trigger(this._element, this.constructor.eventName($xqrD$var$EVENT_HIDE$2));
    if (hideEvent.defaultPrevented) {
      return;
    }
    const tip = this._getTipElement();
    ...
    const complete = () => {
      ...
      $xqrD$var$EventHandler.trigger(this._element, this.constructor.eventName($xqrD$var$EVENT_HIDDEN$2));
    };
    this._queueCallback(complete, this.tip, this._isAnimated());
  }
  update() {
  ...
  _createTipElement(content) {
    ...
    tip.classList.remove($xqrD$var$CLASS_NAME_FADE$2, $xqrD$var$CLASS_NAME_SHOW$2); // todo: on v6 the following can be achieved with CSS only

    tip.classList.add(`bs-${this.constructor.NAME}-auto`);
    const tipId = $xqrD$var$getUID(this.constructor.NAME).toString();
  ...
  _getTemplateFactory(content) {
    if (this._templateFactory) {
      ...
      this._templateFactory = new $xqrD$var$TemplateFactory({
        ...this._config,
  ...
  _getContentForTemplate() {
    return {
      [$xqrD$var$SELECTOR_TOOLTIP_INNER]: this._getTitle()
    };
  }
  _getTitle() {
  ...
  _isAnimated() {
    return this._config.animation || this.tip && this.tip.classList.contains($xqrD$var$CLASS_NAME_FADE$2);
  }
  _isShown() {
    return this.tip && this.tip.classList.contains($xqrD$var$CLASS_NAME_SHOW$2);
  }
  _createPopper(tip) {
    const placement = typeof this._config.placement === 'function' ? this._config.placement.call(this, tip, this._element) : this._config.placement;
    const attachment = $xqrD$var$AttachmentMap[placement.toUpperCase()];
    return $cujj$export$createPopper(this._element, tip, this._getPopperConfig(attachment));
  }
  _getOffset() {
    const {
      offset
  ...
  _setListeners() {
    ...
    for (const trigger of triggers) {
      if (trigger === 'click') {
        $xqrD$var$EventHandler.on(this._element, this.constructor.eventName($xqrD$var$EVENT_CLICK$1), this._config.selector, event => {
          const context = this._initializeOnDelegatedTarget(event);
          context.toggle();
        });
      } else if (trigger !== $xqrD$var$TRIGGER_MANUAL) {
        const eventIn = trigger === $xqrD$var$TRIGGER_HOVER ? this.constructor.eventName($xqrD$var$EVENT_MOUSEENTER) : this.constructor.eventName($xqrD$var$EVENT_FOCUSIN$1);
        const eventOut = trigger === $xqrD$var$TRIGGER_HOVER ? this.constructor.eventName($xqrD$var$EVENT_MOUSELEAVE) : this.constructor.eventName($xqrD$var$EVENT_FOCUSOUT$1);
        $xqrD$var$EventHandler.on(this._element, eventIn, this._config.selector, event => {
          const context = this._initializeOnDelegatedTarget(event);
          context._activeTrigger[event.type === 'focusin' ? $xqrD$var$TRIGGER_FOCUS : $xqrD$var$TRIGGER_HOVER] = true;
          context._enter();
        });
        $xqrD$var$EventHandler.on(this._element, eventOut, this._config.selector, event => {
          const context = this._initializeOnDelegatedTarget(event);
  ...
  _getConfig(config) {
    ...
    for (const dataAttribute of Object.keys(dataAttributes)) {
      if ($xqrD$var$DISALLOWED_ATTRIBUTES.has(dataAttribute)) {
...
const $xqrD$var$Default$2 = {
  ...$xqrD$export$Tooltip.Default,
  content: '',
  offset: [0, 8],
  placement: 'right',
...
const $xqrD$var$DefaultType$2 = {
  ...$xqrD$export$Tooltip.DefaultType,
...
class $xqrD$export$Popover extends $xqrD$export$Tooltip {
  ...
  _getContentForTemplate() {
    return {
      [$xqrD$var$SELECTOR_TITLE]: this._getTitle(),
      [$xqrD$var$SELECTOR_CONTENT]: this._getContent()
    };
  }
  _getContent() {
...
const $xqrD$var$EVENT_CLICK = `click${$xqrD$var$EVENT_KEY$2}`;
const $xqrD$var$EVENT_LOAD_DATA_API$1 = `load${$xqrD$var$EVENT_KEY$2}${$xqrD$var$DATA_API_KEY}`;
...
const $xqrD$var$SELECTOR_TARGET_LINKS = '[href]';
...
const $xqrD$var$SELECTOR_LINK_ITEMS = `${$xqrD$var$SELECTOR_NAV_LINKS}, ${$xqrD$var$SELECTOR_NAV_ITEMS} > ${$xqrD$var$SELECTOR_NAV_LINKS}, ${$xqrD$var$SELECTOR_LIST_ITEMS}`;
...
class $xqrD$export$ScrollSpy extends $xqrD$var$BaseComponent {
  ...
  _configAfterMerge(config) {
    ...
    config.target = $xqrD$var$getElement(config.target) || document.body; // TODO: v6 Only for backwards compatibility reasons. Use rootMargin only
  ...
  _maybeEnableSmoothScroll() {
    ...
    $xqrD$var$EventHandler.off(this._config.target, $xqrD$var$EVENT_CLICK);
    $xqrD$var$EventHandler.on(this._config.target, $xqrD$var$EVENT_CLICK, $xqrD$var$SELECTOR_TARGET_LINKS, event => {
  ...
  _initializeTargetsAndObservables() {
    this._targetLinks = new Map();
    this._observableSections = new Map();
    const targetLinks = $xqrD$var$SelectorEngine.find($xqrD$var$SELECTOR_TARGET_LINKS, this._config.target);
    for (const anchor of targetLinks) {
      // ensure that the anchor has an id and is not disabled
      if (!anchor.hash || $xqrD$var$isDisabled(anchor)) {
        continue;
      }
      const observableSection = $xqrD$var$SelectorEngine.findOne(anchor.hash, this._element); // ensure that the observableSection exists & is visible

      if ($xqrD$var$isVisible(observableSection)) {
  ...
  _process(target) {
    ...
    target.classList.add($xqrD$var$CLASS_NAME_ACTIVE$1);
    this._activateParents(target);
    $xqrD$var$EventHandler.trigger(this._element, $xqrD$var$EVENT_ACTIVATE, {
      relatedTarget: target
    });
  }
  _activateParents(target) {
    // Activate dropdown parents
    if (target.classList.contains($xqrD$var$CLASS_NAME_DROPDOWN_ITEM)) {
      $xqrD$var$SelectorEngine.findOne($xqrD$var$SELECTOR_DROPDOWN_TOGGLE$1, target.closest($xqrD$var$SELECTOR_DROPDOWN)).classList.add($xqrD$var$CLASS_NAME_ACTIVE$1);
      return;
    }
    for (const listGroup of $xqrD$var$SelectorEngine.parents(target, $xqrD$var$SELECTOR_NAV_LIST_GROUP)) {
      // Set triggered links parents as active
      // With both <ul> and <nav> markup a parent is the previous sibling of any nav ancestor
      for (const item of $xqrD$var$SelectorEngine.prev(listGroup, $xqrD$var$SELECTOR_LINK_ITEMS)) {
        item.classList.add($xqrD$var$CLASS_NAME_ACTIVE$1);
      }
    }
  }
  _clearActiveClass(parent) {
    parent.classList.remove($xqrD$var$CLASS_NAME_ACTIVE$1);
    const activeNodes = $xqrD$var$SelectorEngine.find(`${$xqrD$var$SELECTOR_TARGET_LINKS}.${$xqrD$var$CLASS_NAME_ACTIVE$1}`, parent);
    for (const node of activeNodes) {
      node.classList.remove($xqrD$var$CLASS_NAME_ACTIVE$1);
    }
  } // Static
...
$xqrD$var$EventHandler.on(window, $xqrD$var$EVENT_LOAD_DATA_API$1, () => {
  for (const spy of $xqrD$var$SelectorEngine.find($xqrD$var$SELECTOR_DATA_SPY)) {
    $xqrD$export$ScrollSpy.getOrCreateInstance(spy);
  }
});
/**
 * jQuery
...
const $xqrD$var$EVENT_CLICK_DATA_API = `click${$xqrD$var$EVENT_KEY$1}`;
const $xqrD$var$EVENT_KEYDOWN = `keydown${$xqrD$var$EVENT_KEY$1}`;
const $xqrD$var$EVENT_LOAD_DATA_API = `load${$xqrD$var$EVENT_KEY$1}`;
...
const $xqrD$var$CLASS_DROPDOWN = 'dropdown';
const $xqrD$var$SELECTOR_DROPDOWN_TOGGLE = '.dropdown-toggle';
const $xqrD$var$SELECTOR_DROPDOWN_MENU = '.dropdown-menu';
const $xqrD$var$NOT_SELECTOR_DROPDOWN_TOGGLE = ':not(.dropdown-toggle)';
const $xqrD$var$SELECTOR_TAB_PANEL = '.list-group, .nav, [role="tablist"]';
const $xqrD$var$SELECTOR_OUTER = '.nav-item, .list-group-item';
const $xqrD$var$SELECTOR_INNER = `.nav-link${$xqrD$var$NOT_SELECTOR_DROPDOWN_TOGGLE}, .list-group-item${$xqrD$var$NOT_SELECTOR_DROPDOWN_TOGGLE}, [role="tab"]${$xqrD$var$NOT_SELECTOR_DROPDOWN_TOGGLE}`;
const $xqrD$var$SELECTOR_DATA_TOGGLE = '[data-bs-toggle="tab"], [data-bs-toggle="pill"], [data-bs-toggle="list"]'; // todo:v6: could be only `tab`

const $xqrD$var$SELECTOR_INNER_ELEM = `${$xqrD$var$SELECTOR_INNER}, ${$xqrD$var$SELECTOR_DATA_TOGGLE}`;
const $xqrD$var$SELECTOR_DATA_TOGGLE_ACTIVE = `.${$xqrD$var$CLASS_NAME_ACTIVE}[data-bs-toggle="tab"], .${$xqrD$var$CLASS_NAME_ACTIVE}[data-bs-toggle="pill"], .${$xqrD$var$CLASS_NAME_ACTIVE}[data-bs-toggle="list"]`;
/**
 * Class definition
 */
...
class $xqrD$export$Tab extends $xqrD$var$BaseComponent {
  constructor(element) {
    super(element);
    this._parent = this._element.closest($xqrD$var$SELECTOR_TAB_PANEL);
    if (!this._parent) {
  ...
  show() {
    ...
    const hideEvent = active ? $xqrD$var$EventHandler.trigger(active, $xqrD$var$EVENT_HIDE$1, {
      relatedTarget: innerElem
    }) : null;
    const showEvent = $xqrD$var$EventHandler.trigger(innerElem, $xqrD$var$EVENT_SHOW$1, {
  ...
  _activate(element, relatedElem) {
    ...
    this._activate($xqrD$var$getElementFromSelector(element)); // Search and activate/show the proper section
    ...
    const complete = () => {
      ...
      $xqrD$var$EventHandler.trigger(element, $xqrD$var$EVENT_SHOWN$1, {
        relatedTarget: relatedElem
      });
    };
    this._queueCallback(complete, element, element.classList.contains($xqrD$var$CLASS_NAME_FADE$1));
  }
  _deactivate(element, relatedElem) {
    if (!element) {
      return;
    }
    element.classList.remove($xqrD$var$CLASS_NAME_ACTIVE);
    element.blur();
    this._deactivate($xqrD$var$getElementFromSelector(element)); // Search and deactivate the shown section too

    const complete = () => {
      if (element.getAttribute('role') !== 'tab') {
        element.classList.remove($xqrD$var$CLASS_NAME_SHOW$1);
        return;
      }
      element.setAttribute('aria-selected', false);
      element.setAttribute('tabindex', '-1');
      this._toggleDropDown(element, false);
      $xqrD$var$EventHandler.trigger(element, $xqrD$var$EVENT_HIDDEN$1, {
        relatedTarget: relatedElem
      });
    };
    this._queueCallback(complete, element, element.classList.contains($xqrD$var$CLASS_NAME_FADE$1));
  ...
  _keydown(event) {
    if (![$xqrD$var$ARROW_LEFT_KEY, $xqrD$var$ARROW_RIGHT_KEY, $xqrD$var$ARROW_UP_KEY, $xqrD$var$ARROW_DOWN_KEY].includes(event.key)) {
      return;
    }
    event.stopPropagation(); // stopPropagation/preventDefault both added to support up/down keys without scrolling the page

    event.preventDefault();
    const isNext = [$xqrD$var$ARROW_RIGHT_KEY, $xqrD$var$ARROW_DOWN_KEY].includes(event.key);
    const nextActiveElement = $xqrD$var$getNextActiveElement(this._getChildren().filter(element => !$xqrD$var$isDisabled(element)), event.target, isNext, true);
    if (nextActiveElement) {
      nextActiveElement.focus({
        preventScroll: true
      });
      $xqrD$export$Tab.getOrCreateInstance(nextActiveElement).show();
    }
  }
  _getChildren() {
    // collection of inner elements
    return $xqrD$var$SelectorEngine.find($xqrD$var$SELECTOR_INNER_ELEM, this._parent);
  }
  _getActiveElem() {
  ...
  _setInitialAttributesOnTargetPanel(child) {
    const target = $xqrD$var$getElementFromSelector(child);
    if (!target) {
      return;
    }
  ...
  _toggleDropDown(element, open) {
    ...
    if (!outerElem.classList.contains($xqrD$var$CLASS_DROPDOWN)) {
      return;
    }
    const toggle = (selector, className) => {
      const element = $xqrD$var$SelectorEngine.findOne(selector, outerElem);
      if (element) {
        element.classList.toggle(className, open);
      }
    };
    toggle($xqrD$var$SELECTOR_DROPDOWN_TOGGLE, $xqrD$var$CLASS_NAME_ACTIVE);
    toggle($xqrD$var$SELECTOR_DROPDOWN_MENU, $xqrD$var$CLASS_NAME_SHOW$1);
  ...
  _elemIsActive(elem) {
    return elem.classList.contains($xqrD$var$CLASS_NAME_ACTIVE);
  } // Try to get the inner element (usually the .nav-link)

  _getInnerElement(elem) {
    return elem.matches($xqrD$var$SELECTOR_INNER_ELEM) ? elem : $xqrD$var$SelectorEngine.findOne($xqrD$var$SELECTOR_INNER_ELEM, elem);
  } // Try to get the outer element (usually the .nav-item)

  _getOuterElement(elem) {
    return elem.closest($xqrD$var$SELECTOR_OUTER) || elem;
  } // Static
...
$xqrD$var$EventHandler.on(document, $xqrD$var$EVENT_CLICK_DATA_API, $xqrD$var$SELECTOR_DATA_TOGGLE, function (event) {
  ...
  $xqrD$export$Tab.getOrCreateInstance(this).show();
});
/**
 * Initialize on focus
 */
...
$xqrD$var$EventHandler.on(window, $xqrD$var$EVENT_LOAD_DATA_API, () => {
  for (const element of $xqrD$var$SelectorEngine.find($xqrD$var$SELECTOR_DATA_TOGGLE_ACTIVE)) {
    $xqrD$export$Tab.getOrCreateInstance(element);
  }
});
/**
 * jQuery
...
class $xqrD$export$Toast extends $xqrD$var$BaseComponent {
  ...
  show() {
    ...
    $xqrD$var$reflow(this._element);
    this._element.classList.add($xqrD$var$CLASS_NAME_SHOW, $xqrD$var$CLASS_NAME_SHOWING);
  ...
  hide() {
    ...
    const complete = () => {
      ...
      this._element.classList.remove($xqrD$var$CLASS_NAME_SHOWING, $xqrD$var$CLASS_NAME_SHOW);
  ...
  isShown() {
    return this._element.classList.contains($xqrD$var$CLASS_NAME_SHOW);
  } // Private
...
$(document).ready(function () {
  ...
  window.onscroll = function (e) {
    ...
    if (variation > 0) {
      ...
      document.querySelector("#top").style.top = "0px";
      document.querySelector("#home0").style.height = "98vh";
    }
    if (variation < 0) {
